...
- API responses: explicitly *not* cached by browsers
...
If you'd like, I can:
- Add `@EnableCaching` and a `CacheManager` bean to the backend and add example `@Cacheable`/`@CacheEvict` annotations for `CategoryServiceImpl` (safe small PR), or
- Add monitoring endpoints to inspect cache stats.

Tell me which option you prefer and I can open a PR with the code changes. 👍